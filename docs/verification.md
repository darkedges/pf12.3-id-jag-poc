...
The runner prints an equivalent Windows `curl.exe` request before issuing the fixture exchange. Its client secret and subject token are placeholders, so copying the displayed command requires supplying those values separately. The example uses `--insecure` only for the local self-signed endpoint.
